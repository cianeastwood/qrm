...
#### Notes
1. `--single-test-env`: Ignores the costly and less common model-selection method of `LeaveOneOutSelectionMethod` (focusing on 
`IIDAccuracySelectionMethod` and `OracleSelectionMethod` instead).
2. `MyLauncher`: DomainBed requires you to implement a launcher for your cluster. We have 
included our
[submitit launcher](domainbed/command_launchers.py#L41) 
for launching jobs on a slurm cluster with submitit, in case it is a useful starting point. 
Slurm parameters will need to be changed for your cluster.
